...
// Comparator to sort by (minimum - actual) descending
int compareTasks(const void *a, const void *b) {
    ...
    // Sort descending: if diffB > diffA, return positive
    return diffB - diffA;
}

int minimumEffort(int** tasks, int tasksSize, int* tasksColSize) {
    // Sort the tasks using the greedy criteria
    ...
    for (int i = 0; i < tasksSize; i++) {
        ...
        // If current energy isn't enough to start this task
        if (current_energy < minimum) {
            // Add exactly enough to reach the requirement
            min_initial_energy += (minimum - current_energy);
            current_energy = minimum;
        }
        
        // Subtract the energy actually consumed
