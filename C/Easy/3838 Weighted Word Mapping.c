...
char* mapWordWeights(char** words, int wordsSize, int* weights, int weightsSize) {
    // Allocate memory for the resulting string + null terminator
    char* result = (char*)malloc((wordsSize + 1) * sizeof(char));
    
    for (int i = 0; i < wordsSize; i++) {
        int currentWeight = 0;
        // Sum weights for each character in the current word
        for (int j = 0; words[i][j] != '\0'; j++) {
            currentWeight += weights[words[i][j] - 'a'];
        }
        
        // Map the result modulo 26 to reverse alphabetical order
        int val = currentWeight % 26;
        result[i] = 'z' - val;
    }
