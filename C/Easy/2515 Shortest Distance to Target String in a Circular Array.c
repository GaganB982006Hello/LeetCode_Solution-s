...
int closestTarget(char ** words, int wordsSize, char * target, int startIndex) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (strcmp(words[i], target) == 0) {
            ...
            // 1. Direct distance: absDist
            // 2. Wrap-around distance: n - absDist
            int currentDist = MIN(absDist, n - absDist);
            
            // If this is our first match or it's smaller than previous matches
